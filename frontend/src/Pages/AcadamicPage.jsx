      ...
      <div className='mx-auto flex flex-wrap gap-5 mt-5'>
        <div className='border-2 border-[#545454] rounded-[30px] flex flex-col gap-3 max-w-[620px]'>
          <div className=' h-[400px] p-4 m-2'>
            {data.length > 0 ? <p className='text-[#d8973c] text-[40px] itim '>{qoute}</p>:<p className='text-[#d8973c] text-[40px] itim'>“Success is not final; failure is not fatal: It is the courage to continue that counts.”</p>}
          </div>
          <div>{data.length > 0 ? <p className='text-[#63a73a] text-[40px] itim p-4 '>-{author}</p> :<p className='text-[#63a73a] text-[40px] itim p-4 '>-Winston Churchill</p>}</div>
        </div>
